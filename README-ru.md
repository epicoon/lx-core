...
3. Чтобы развернуть платформу в проекте, нужно запустить php-скрипт `vendor/lx/lx-core/lx-install`.<br>
   ...
   * services - каталог для сервисов приложения (подробности ниже). Необязателен, работа с сервисами настраивается в конфигурации приложения.
...
* <a name="arch-module"><h3>Модули</h3></a>
  Модуль является элементом сервиса, представляющим собой совокупность логики, выполняющейся на клиенте, графического интерфейса, ее обслуживающего и серверной части, представленной респондентами (AJAX-контроллеры, работающие в контексте конкретных модулей). По своей идее напоминает SPA.
  ...
  * в коде своих блоков (PHP-файлы представлений) модуль доступен в виде контекстной переменной `$Module`
  * в своем JS-коде модуль доступен в виде контекстной переменной `Module`
  ...
  * JS-код модуля. Основных файлов для исполнения два - один будет выполняться до разворачивания модуля (путь к файлу определяет ключ конфигурации `jsMain`), другой после (путь к файлу определяет ключ конфигурации `jsBootstrap`).
  ...
  * Представление. Рендеринг модуля начинается с корневого блока, код которого описывается в корневом файле представления (путь к файлу определяет ключ конфигурации `view`).
  ...
  * CSS-ресурсы (ключ конфигурации `css`). Можно задать каталог, в котором будут лежать css-файлы модуля. При загрузке модуля эти файлы будут использоваться автоматически.<br>
  ...
  ```yaml
  # JS-код, который выполнится до разворачивания модуля, будет находиться
  # в файле 'frontend/_bootstrap.js' относительно корневого каталога модуля
  jsBootstrap: frontend/_bootstrap.js
  # JS-код, который выполнится после разворачивания модуля, будет находиться
  # в файле 'frontend/_main.js' относительно корневого каталога модуля
  ...
  # Корневым файлом графического интерфейса будет 'view/_root.php',
...
* <a name="arch-block"><h3>Блоки</h3></a>
  ...
  Варианты как может выглядеть путь к блоку с именем `blockName`:
  ...
  * path/to/block/blockName/_blockName.php
  ...
  Блок может иметь свой js-код, управляющий описываемым фрагментом графического интерфейса (как раз для этого нужен вариант блока-каталога). В этом случае требования к имени файла такие же как для файла представления (но с расширением `.js`).<br>
  Пример:
  * path/to/block/blockName/blockName.php
  ...
  * path/to/block/blockName/_blockName.js
  ...
  При написании PHP-кода блока доступны две контекстные переменные:
  ...
  * clientParams - объект с полями, которые можно задать на PHP, когда блок загружается в элемент
  ...
  // Вкладываем блок - в данном случае имя это путь относительно файла
  // описываемого в данный момент блока
  ...
  //                       на стороне клиента в контекстном объекте 'clientParams'
  $box2->setBlock([
    'path' => 'blockName2',
    'renderParams' => [],
    'clientParams' => [],
  ...
  При выполнении js-кода блока в браузере, доступен контекст выполнения родительского блока, но недоступны контексты выполнения вложенных блоков. Например, в блок `A` вложен блок `B`, в блок `B` вложен блок `C`: `A -> B -> C`. В блоках объявлены соответственно переменные `a`, `b` и `c`. Тогда в блоке `A` будет доступна только переменная `a`. В блоке `B` будут доступны переменные `a` и `b`. В блоке `C` будут доступны переменные `a`, `b` и `c`.

  Путь к корневому блоку модуля задается в lx-конфигурации модуля по ключу `view`.
