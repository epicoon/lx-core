...
class ModuleBuilder {
	...
	public function moduleInfo() {
		if ($this->_moduleInfo === null) {
			 ...
			 * Это только для того, чтобы модуль знал о своих собственных зависимостях
			 * и, если модуль сброшен, виджеты тоже можно было сбросить
			 * На ajax-дозагрузку виджетов по зависимостям это не влияет - для этого строится
			 * единая карта зависимостей для всех отрендеренных модулей
			 */
