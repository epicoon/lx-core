...
(function () {
	...
	function valueToWidgetWithoutBind(widget, value) {
		if (widget.hasMethod('setValueWithoutBind'))
			widget.setValueWithoutBind(value);
		else if (widget.hasMethod('value'))
			widget.value(value);
		else if (widget.hasMethod('text'))
			widget.text(value);
	}

	// Метод непосредственного помещения значения в виджет
	function valueToWidget(widget, value) {
		if (widget.hasMethod('value'))
			widget.value(value);
		else if (widget.hasMethod('text'))
			widget.text(value);
	}
	...
	var bindableBehavior = function(constructor, fields, extraFields) {
		var hasBehavior = lx.Behavior.hasBehavior(constructor, arguments.callee);

		lx.Behavior.setterListener(constructor, fields, extraFields);

		if (!hasBehavior) {
			constructor.afterSet(function(val, name) { action(this, name, val); });
			constructor.onSetterFail(function(val, name) { action(this, name, this[name]); });
		}
	};
})();
