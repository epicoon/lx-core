...
lx.Behavior.setterListener = function(constructor, fields=[], extraFields={}) {
...
var setterBehavior = function(constructor, fields, extraFields) {
	if (!lx.Behavior.hasOwnBehavior(constructor, arguments.callee)) {
	...
	if (!lx.Behavior.hasBehavior(constructor, arguments.callee)) {
		lx.Behavior.register(constructor, arguments.callee);

		constructor.beforeSet = function(name, func) {
			if (name.isObject) {
		...
		constructor.afterSet = function(name, func) {
			if (name.isObject) {
	...
	var f = (function(){
		...
		function run(arr) {
			for (var i=0, l=arr.len; i<l; i++) {
				var res = arr[i].call(this, val, name);
				if (res === false) {
					info.fail.each((func)=> func.call(this, val, name));
					return false;
	...
	/**
	 * Может ассоциировать ключ с любым полем, в т.ч. находящемся в используемых объектах
...
*/
lx.Behavior.methodListener = function(obj) {
	//todo !!!!
	if (obj.beforeMethod) return;
	...
	Object.defineProperty(obj, "beforeMethod", {
		enumerable: false,
	...
	Object.defineProperty(obj, "afterMethod", {
		enumerable: false,
		value: function(funcName, func) {
